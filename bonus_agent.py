...
class Agent:
    ...
    def perceive(self) -> None:
      ...
      {', '.join(tool for tool, data in self.env.tools.items() if data['used'])}"""
        ...
        self.message_history.append({"role": "user", "content": env_description})
    ...
    def retrieve(self) -> str:
        ...
        memories = (
            open(f"cs222_assignment_1_bonus/{self.name}/memory/cake.txt", "r")
            .read()
            .split("\n\n")
        )
        ...
        for block in memories:
            if "cake" in block.lower() and (
                "ingredient" in block.lower() or "step" in block.lower()
            ):
                recipe += block + "\n\n"
