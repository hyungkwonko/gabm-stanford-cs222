...
def normalize_dict_floats(
    d: Dict[Any, float], target_min: float, target_max: float
) -> Dict[Any, float]:
    """
...
class MemoryStream:
    def __init__(self, nodes: List[Dict[str, Any]], embeddings: Dict[str, List[float]]):
    ...
    def reflect(
        ...
        records = self.retrieve([anchor], time_step, retrieval_count, verbose=DEBUG)[
            anchor
        ]
        ...
        for count, reflection in enumerate(reflections):
            self._add_node(
                time_step, "reflection", reflections[count], scores[count], record_ids
            )
...
def extract_relevance(
    seq_nodes: List[ConceptNode], embeddings: Dict[str, List[float]], focal_pt: str
) -> Dict[int, float]:
    """
    ...
    relevance_out = dict()
    for _, seq_node in enumerate(seq_nodes):
        node_embedding = embeddings[seq_node.content]
        cos_val = cos_sim(node_embedding, focal_embedding)
        relevance_out[seq_node.node_id] = cos_val
...
def run_gpt_generate_reflection(
    ...
    if reflection_count > 1:
        prompt_lib_file = (
            f"{LLM_PROMPT_DIR}/generative_agent/memory_stream/reflection/batch_v1.txt"
        )
    else:
...
def generate_reflection(
    records: List[ConceptNode], anchor: str, reflection_count: int
) -> List[str]:
    """Generate reflections based on given records and anchor."""
    records = [i.content for i in records]
    return run_gpt_generate_reflection(
        records, anchor, reflection_count, "1", LLM_VERS
    )[0]
