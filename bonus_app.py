...
def next_step():
    ...
    if current_step < max_steps and not isabella.env._all_steps_completed():
        ...
        agent_message, attempted_actions, executed_actions, feedbacks = (
            isabella.baking_step()
        )
        ...
        if isabella.env._all_steps_completed():
            final_message = (
                ...
                if isabella.env.check_final_ingredients()
                else "Hmm... it seems we made some mistakes with the ingredients and the cake tastes... not so good. Let's try again next time!"
            )
            isabella.message_history.append(
